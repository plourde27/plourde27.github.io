...
renderer.render(scene, camera);
  ...
  function animate() {
    ...
    if (keys[87]) {
      camera.position.x += Math.cos(theta) * 2;
      camera.position.z += Math.sin(theta) * 2;
    }
    ...
    for (var i = 0 ; i < dwalls.length ; i++) {
      ...
      var m1 = (py2 - py1) / (px2 - px1 + 0.0000001);
      var b1 = py2 - px2 * m1;

      var m2 = (wy2 - wy1) / (wx2 - wx1 + 0.0000001);
      var b2 = wy2 - wx2 * m2;

      var ix = (b2 - b1) / (m1 - m2 + 0.00000001);
      var iy = m1 * ix + b1;
      ...
      var OF = 10;
      var wallDist = Math.sqrt((px2-ix)*(px2-ix)+(py2-iy)*(py2-iy));
      var lfd = Math.sqrt((px2-wx1)*(px2-wx1)+(py2-wy1)*(py2-wy1));
      var rfd = Math.sqrt((px2-wx2)*(px2-wx2)+(py2-wy2)*(py2-wy2));
      var od = Math.sqrt((wx2-wx1)*(wx2-wx1)+(wy2-wy1)*(wy2-wy1));
      if (i==0 && keys[88]) {console.log(wallDist + " " + owd[i]);
    console.log(Math.abs((lfd + rfd) - od));}
      //if (wallDist < 5 && ix >= Math.min(wx1,wx2) - OF && ix <= Math.max(wx1,wx2) + OF && ix >= Math.min(px1,px2) - OF && ix <= Math.max(px1,px2) + OF && iy >= Math.min(wy1,wy2) - OF && iy <= Math.max(wy1,wy2) + OF && iy >= Math.min(py1,py2) - OF && iy <= Math.max(py1,py2) + OF) {
      if (wallDist < OF && Math.abs((lfd + rfd) - od) < OF*2 && wallDist <= owd[i]) {
